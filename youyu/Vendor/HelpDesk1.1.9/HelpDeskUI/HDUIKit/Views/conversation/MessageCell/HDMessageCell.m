                         ...
                         model:(id<HDIMessageModel>)model
{
    ...
    _hasRead.text = NSEaseLocalizedString(@"阅读", @"Read");
        ...
        switch (model.bodyType) {
            case EMMessageBodyTypeText:
            {
                ...
                switch (extMsgType) {
                    ...
                    case HExtEvaluationMsg:
                    {
                        _bubbleView.evaluateTitle.attributedText = [[NSAttributedString alloc] initWithString:NSLocalizedString(@"请评估我们的服务", @"please evaluate my service")];
                    }
                        break;
                    case HExtToCustomServiceMsg: {
    ...
    switch (model.bodyType) {
        case EMMessageBodyTypeText: {
            ...
            switch (extMsgType) {
                ...
                case HExtEvaluationMsg:
                {
                    NSAttributedString *text = [[NSAttributedString alloc] initWithString: NSLocalizedString(@"请评估我们的服务", @"please evaluate my service")];
                    CGRect rect = [text boundingRectWithSize:CGSizeMake(bubbleMaxWidth - 25, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading  context:nil];
                    height += (rect.size.height > 20 ? rect.size.height : 20) + 21;
                    height += 50;
                    return height;
