...
- (void)setLeftBarBtnItem {
    QMUINavigationButton *btn = [[QMUINavigationButton alloc] initWithType:QMUINavigationButtonTypeBack];
    self.navigationItem.leftBarButtonItem = [UIBarButtonItem qmui_itemWithButton:btn target:self action:@selector(backItemClicked)];
}
...
- (void)didHDStartRecordingVoiceAction:(UIView *)micView
{
    ...
    [self _canRecordCompletion:^(HDRecordResponse recordResponse) {
        switch (recordResponse) {
            ...
            case HDCanNotRecord:
            {
                UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"提示", @"Prompt") message:NSLocalizedString(@"没有录制权限", @"No recording permission") delegate:nil cancelButtonTitle:NSLocalizedString(@"确定", @"OK") otherButtonTitles:nil, nil];
                [alertView show];
            }
                break;
            default:
        ...
        if ([HMessageHelper getMessageExtType:message] == HExtToCustomServiceMsg) {
            ...
            [[HChatClient sharedClient].chatManager sendMessage:aHMessage progress:nil completion:^(HMessage *aMessage, HError *aError)
            {
                ...
                if (!aError) {
                    ...
                    [weakSelf showHint:NSLocalizedString(@"请求失败，请确认连接状态！", @"Transfer to the artificial customer service request failed, please confirm the connection status!")];
                }
            }];
        }
    }
...
- (void)commitSatisfactionWithControlArguments:(ControlArguments *)arguments type:(ControlType *)type evaluationTagsArray:(NSMutableArray *)tags{
    ...
    __weak typeof(self) weakself = self;
    [self showHudInView:self.view hint:NSLocalizedString(@"提交", @"Comment Submit.")];
    [[HChatClient sharedClient].chatManager sendMessage:message progress:nil completion:^(HMessage *aMessage, HError *aError) {
        ...
        if (!aError) {
            ...
            [weakself.tableView reloadData];
            [weakself showHint:NSLocalizedString(@"提交成功", @"Add comment successful.")];
        } else {
            [weakself showHint:NSLocalizedString(@"提交失败", @"Add comment fail.")];
        }
