...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = NSLocalizedString(@"满意度评价", @"satisfaction evaluation");
...
- (UILabel*)nickLabel
{
    if (_nickLabel == nil) {
        ...
        _nickLabel.text = NSLocalizedString(@"昵称", @"Nickname");
...
- (UILabel*)textLabel
{
    if (_textLabel == nil) {
        ...
        _textLabel.text = NSLocalizedString(@"please evaluate my service", @"please evaluate my service");
...
- (UIButton*)commitBtn
{
    if (_commitBtn == nil) {
        ...
        [_commitBtn setTitle:NSLocalizedString(@"提交", @"commit") forState:UIControlStateNormal];
...
- (void)commit
{
    if (!_starRateView.isTap && _starRateView.scorePercent != 1) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"" message:NSLocalizedString(@"请先评价", @"please evaluate first") delegate:self cancelButtonTitle:NSLocalizedString(@"取消", @"Cancel") otherButtonTitles:NSLocalizedString(@"确定", @"Ok"), nil];
        [alert show];
        return;
    }
        ...
        if ([self.messageModel.message.ext objectForKey:kMesssageExtWeChat]) {
            ...
            if ([weichat objectForKey:kMesssageExtWeChat_ctrlArgs]) {
                ...
                if (self.evaluationTagsArray.count == 0 && self.evaluationTagView.evaluationDegreeModel.appraiseTags.count>0) {
                    [self showHint:NSLocalizedString(@"选择至少一个标签", @"Select at least one tag!")];
                } else {
