...
- (void)navigationController:(UINavigationController *)navigationController willShowViewController:(UIViewController *)viewController animated:(BOOL)animated {
    if ([viewController isKindOfClass:NSClassFromString(@"PLUIImageViewController")] && self.editMode && ([navigationController.viewControllers count] == 3)) {
        ...
        position = plCropOverlay.center.y - diameter / 2;
        ...
        solidLine.strokeColor = [UIColor colorWithWhite:0.8 alpha:0.8].CGColor;
        solidLine.fillColor = [UIColor clearColor].CGColor;

        CGRect rect = CGRectMake(0, position, diameter, diameter);
        rect = CGRectEdgeInsets(rect, UIEdgeInsetsMake(2.5, 2.5, 2.5, 2.5));
        CGPathAddEllipseInRect(solidPath, nil, rect);
        solidLine.path = solidPath;
        CGPathRelease(solidPath);
        [viewController.view.layer addSublayer:solidLine];
    }
}
